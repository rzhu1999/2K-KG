...
class Canonicalizer_ct {
	constructor(k_tree) {
		Object.assign(this, {
			tree: k_tree,
			blanks: {},
			issuer: new Issuer('_:r'),
			quads: [...k_tree.ct_quads()],
		});
	}
	...
	normalize() {
		...
		for(let p_blank of Object.keys(h_hashes).sort()) {
			...
			for(let g_hash_result of a_paths.sort(F_SORT_CANONICALIZER_HASH)) {
				for(let sc1_existing in g_hash_result.issuer.existing) {
...
class DatasetTree extends stream.Duplex {
	...
	constructor(g_config={}, h_quad_tree=null) {
		...
		this.once('finish', () => {
			...
			if(this.push_waiting) {
				this._read(this.push_waiting);
			}
		});
		...
		Object.assign(this, {
			...
			leaf_blanks: {},
			prefixes: h_prefixes,
			prefixes_flushed: false,
			debug: g_config.debug || false,
			quad_generator: null,
			graph_generator: null,
			graph_iterator: null,
			subject_generator: null,
			transform_canonicalize: g_config.canonicalize || false,
			transform_tree: this,
			push_waiting: 0,
		});
	...
	_read(n_quads) {
		...
		if(this._writableState.finished) {
			...
			if(this.transform_canonicalize) {
				k_tree = this.canonicalize();
			}

			// start new quad iterator
			this.quad_generator = k_tree.quads();
		...
		// not ready yet
			...
			this.push_waiting = n_quads;
		}
	}

	// resume default read: quad mode
	resume_read(n_quads) {
		let dg_quads = this.quad_generator;
	...
	read_graphy_after_finished(n_size) {
		...
		// transform canonicalize option
		if(this.transform_canonicalize) {
			k_tree = this.transform_tree = this.canonicalize();
		}

		// initialize generator(s) & iterator
		let dg_graphs = this.graph_generator = k_tree.c1_graphs();
		this.graph_iterator = dg_graphs.next();
	...
	resume_read_graphy(n_size) {
		let {
			graph_generator: dg_graphs,
			graph_iterator: di_graph,
			subject_generator: dg_subjects,
			transform_tree: k_tree,
		} = this;

		let h_quads = k_tree.quad_tree;
		...
		while(!di_graph.done) {
			...
			if(!dg_subjects) {
				dg_subjects = this.subject_generator = k_tree.c1_subjects(sv1_graph);
			}
			...
			di_graph = this.graph_iterator = dg_graphs.next();
		}
	...
	* c1_graphs() {
		for(let sv1_graph in this.quad_tree) {
			yield sv1_graph;
		}
	}

	* c1_subjects(sv1_graph) {
		let h_subjects = this.quad_tree[sv1_graph];
		for(let sv1_subject in h_subjects) {
			yield sv1_subject;
		}
	}
	...
	* pairs(sv1_graph, sv1_subject) {
	...
	match(z_subject=null, z_predicate=null, z_object=null, z_graph=null) {
		...
		if(z_graph) {
			// convert graph to c1
			let sc1_graph_target = factory.from.term(z_graph).concise();
			...
			if(z_subject) {
				...
				let sc1_subject_target = factory.from.term(z_subject).concise();
				...
				if(z_predicate) {
					...
					let sc1_predicate_target = factory.from.term(z_predicate).concise();
					...
					if(z_object) {
						...
						let sc1_object_target = factory.from.term(z_object).concise();
	...
	equivalent(k_other) {
		...
		// compare digests
		return this.canonicalize().string === k_other.canonicalize().string;
	}
	...
	union(k_other) {
		...
		return new DatasetTree({
			...
			prefixes: Object.assign({}, k_other.prefixes, this.prefixes),
		}, h_quads_u);
	}
	...
	minus_subset(k_subset) {
		...
		return new DatasetTree({
			// merge prefixes; this instance has precedence
			prefixes: Object.assign({}, k_subset.prefixes, this.prefixes),
		}, h_quads_r);
	}
