...
class reader {
	...
	// advance index
				...
				this.emit('exit', this.graph);

				// reset graph
				this.graph = KT_DEFAULT_GRAPH;
	...
	post_blank_subject() {
		...
		if('.' === s[i]) {
			...
			this.i = R_WS.lastIndex;
							...
							// not inside block
			if(KT_DEFAULT_GRAPH === this.graph) {
				return this.block();
			}
				// inside block
			else {
...
// absolute iri
				if(!this.base_url || RT_IRI_ABSOLUTE.test(s_iri)) {
		// set graph
					this.graph = this.named_node(s_iri);
				}
	// relative iri
				else {
					this.graph = this.named_node(uri.resolve(this.base_url, s_iri));
				}

				this.emit('enter', this.graph);
...
// save state before pausing
		this.state = this.graph_or_subject;
...
// save state before pausing
		this.state = this.graph_or_subject_property_list;
...
// save state before pausing
		this.state = this.graph_or_subject_anon;
...
// invalid prefix
				...
				// make subject key
				this.graph = this.named_node(this.prefixes[s_prefix_id] + m_prefixed_named_e_graph[2]);
...
// invalid prefix
				...
				// make subject key
									this.graph = this.named_node(this.prefixes[s_prefix_id] + s_suffix);
...
// save state before pausing
		this.state = this.graph_post_name;
	...
	post_object() {
		...
		// '.' end of statement
			...
			// assert not nested
				...
				return (KT_DEFAULT_GRAPH === this.graph)? this.block(): this.statement();
